-- chunkname: @script/lua/unit_extensions/ai/ai_inventory_extension.lua

class("AIInventoryExtension")
...
AIInventoryExtension.init = function (self, unit, extension_init_data)
	...
	for i = 1, items_n do
		...
		local item_unit = spawn_manager:spawn_unit(item_unit_name)
...
AIInventoryExtension.destroy = function (self)
	...
	for i = 1, inventory_items_n do
		...
		spawn_manager:mark_for_deletion(item_unit)
...
AIInventoryExtension.disable_inventory_scene_queries = function (self)
	for _, item_unit in pairs(self.inventory_item_units) do
		for j = 1, Unit.num_actors(item_unit) do
			local actor = Unit.actor(item_unit, j)
