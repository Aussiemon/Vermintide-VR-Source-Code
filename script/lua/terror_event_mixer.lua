-- chunkname: @script/lua/terror_event_mixer.lua

require("script/lua/utils/conflict_utils")
...
TerrorEventMixer.init_functions = {
	...
	spawn_at_raw = function (event, element, t)
		if Managers.player.is_server then
			...
			conflict_director:spawn_at_raw_spawner(Breeds[element.breed_name], element.spawner_id)
		end
	end,
	...
	event_horde = function (event, element, t)
		...
		local horde_data = spawn_manager:event_horde(t, element.spawner_id, element.composition_type, element.limit_spawners, element.horde_silent, nil, element.target)
	...
	flow_event = function (event, element, t)
		...
		local world = TerrorEventMixer.world
		local level = World.get_data(world, "current_level")

		Level.trigger_event(level, flow_event)
	end,
...
TerrorEventMixer.run_functions = {
	spawn = function (event, element, t, dt)
		local conflict_director = Managers.state.conflict
		local unit = conflict_director:spawn_one(Breeds[element.breed_name], event.optional_pos and event.optional_pos:unbox())
	...
	horde = function (event, element, t, dt)
		...
		if living_horde > element.peak_amount then
