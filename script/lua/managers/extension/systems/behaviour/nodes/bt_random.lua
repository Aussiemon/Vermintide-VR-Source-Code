-- chunkname: @script/lua/managers/extension/systems/behaviour/nodes/bt_random.lua

require("script/lua/managers/extension/systems/behaviour/nodes/bt_node")
class("BTRandom", "BTNode")
...
BTRandom.leave = function (self, unit, blackboard, t)
...
BTRandom.run = function (self, unit, blackboard, t)
	...
	if running_child then
		...
		local result = running_child:run(unit, blackboard, t)
	...
	for i = 1, num_children do
		...
		if child:condition(blackboard) then
			...
			local result = child:run(unit, blackboard, t)
