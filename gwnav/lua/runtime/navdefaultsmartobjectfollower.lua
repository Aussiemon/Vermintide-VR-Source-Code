-- chunkname: @gwnav/lua/runtime/navdefaultsmartobjectfollower.lua
...
NavDefaultSmartObjectFollower.get_smartobject_type = function (self, next_smartobject_interval)
	local smartobject_id = GwNavSmartObjectInterval.smartobject_id(next_smartobject_interval)

	return self.navbot.navworld:get_smartobject_type(smartobject_id)
end
	...
	if approachingDistance > Vector3.distance(entrance_pos, botpos) then
		...
		if self.navbot.is_smartobject_driven == false then
			...
			if in_manual_control == true then
				self:start_follow(exit_pos, self.navbot.config.smartobject_jump_anim_event)
