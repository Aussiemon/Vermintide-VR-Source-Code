-- chunkname: @gwnav/lua/runtime/navbotconfiguration.lua
...
NavBotConfiguration.init = function (self, bot_configuration_unit)
	...
	self.database_id = self:get_configuration_value(0, self.config_name, "database_id")
	...
	self.enable_crowd_dispersion = self:get_configuration_value(false, self.config_name, "enable_crowd_dispersion")
	self.crowd_dispersion_repath_distance = self:get_configuration_value(5, self.config_name, "crowd_dispersion_repath_distance")
	self.crowd_dispersion_no_repath_distance = self:get_configuration_value(1, self.config_name, "crowd_dispersion_no_repath_distance")
	...
	self.smartobject_jump_anim_event = self:get_configuration_value("jump", self.config_name, "smartobject", "jump_anim_event")
end

NavBotConfiguration.configure_bot = function (self, bot)
	bot:set_use_avoidance(self.enable_avoidance)
	bot:set_use_channel(self.use_channel)
	bot:set_avoidance_computer_config(self.avoidance_angle_span, self.avoidance_minimal_time_to_collision, self.avoidance_sample_count)
	bot:set_avoidance_collider_collector_config(self.collider_collector_half_height, self.collider_collector_radius, self.collider_collector_frame_delay)
	bot:set_avoidance_behavior(self.avoidance_enable_slowing_down, self.avoidance_enable_force_passage, self.avoidance_enable_stop, self.avoidance_stop_wait_time_s, self.avoidance_force_passage_time_limit_s, self.avoidance_wait_passage_time_limit_s)
	bot:set_channel_radius(self.channel_radius)
	bot:set_channel_advanced_config(self.turn_sampling_angle, self.channel_smoothing_angle, self.min_distance_between_gates, self.max_distance_between_gates)
	bot:set_spline_trajectory_config(self.animation_driven, self.max_distance_to_spline_position, self.spline_length, self.spline_distance_to_borders, self.spline_recomputation_distance, self.target_on_spline_distance)
	bot:set_propagation_box(self.pathfinder_propagation_box_extent)
	bot:set_outside_navmesh_distance(self.pathfinder_from_outside_navmesh_distance, self.pathfinder_to_outside_navmesh_distance)
	bot:set_navtag_layer_cost_table(self.navtag_layer_cost_table)
end
