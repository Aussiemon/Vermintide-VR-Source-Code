-- chunkname: @gwnav/lua/runtime/navbot.lua
...
local NavDefaultSmartObjectFollower = safe_require("gwnav/lua/runtime/navdefaultsmartobjectfollower")
...
NavBot.init = function (self, navworld, unit, config)
	...
	local gwnavdatabase = GwNavWorld.database(navworld.gwnavworld, self.config.database_id)

	self.gwnavbot = GwNavBot.create(gwnavdatabase, self.config.height, self.config.radius, self.config.speed, p)

	GwNavBot.enable_crowd_dispersion(self.gwnavbot, self.config.crowd_dispersion_repath_distance, self.config.crowd_dispersion_no_repath_distance)
...
NavBot.set_destination = function (self, destination)
	self.repath_due_to_crowd_dispersion = false
...
NavBot.set_channel_radius = function (self, channel_radius)
	GwNavBot.set_channel_radius(self.gwnavbot, channel_radius)
end

NavBot.set_channel_advanced_config = function (self, turn_sampling_angle, channel_smoothing_angle, min_distance_between_gates, max_distance_between_gates)
	GwNavBot.set_channel_advanced_config(self.gwnavbot, turn_sampling_angle, channel_smoothing_angle, min_distance_between_gates, max_distance_between_gates)
end
...
NavBot.update_crowd_dispersion = function (self)
	...
	if crowd_dispersion_action == 1 then
		if not GwNavBot.is_computing_new_path(self.gwnavbot) then
			...
			self.repath_due_to_crowd_dispersion = true
		end
	elseif crowd_dispersion_action == 2 and GwNavBot.is_computing_new_path(self.gwnavbot) and self.repath_due_to_crowd_dispersion == true then
		self.repath_due_to_crowd_dispersion = false

		GwNavBot.cancel_async_path_computation(self.gwnavbot)
	end
end
