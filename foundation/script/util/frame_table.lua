-- chunkname: @foundation/script/util/frame_table.lua
...
local function alloc_table()
	...
	if frame_table_counter >= frame_table_max_size and DEBUG_FRAME_TABLES then
	...
	if frame_table_counter == frame_table_max_size then
		...
		for i = frame_table_counter, frame_table_max_size do
			frame_table_collection_buffers[1][i] = {}
			frame_table_collection_buffers[2][i] = {}
		end
	end
...
local function clear_tables()
	...
	for i = 1, frame_table_counter do
