-- chunkname: @foundation/script/managers/package/package_manager.lua

local function debug_print(format, ...)
	if true or script_data.package_debug then
...
PackageManager.load = function (self, package_name, reference_name, callback, asynchronous, prioritize)
	debug_print("Load:  %s, %s, %s", package_name, reference_name, asynchronous and "async-read" or "sync-read")
...
PackageManager.force_load = function (self, package_name)
	...
	local package = assert(self._asynch_packages[package_name], "Package %q is not being loaded", package_name)
...
PackageManager.force_load_queued_package = function (self, package_name)
	...
	local package = assert(self._queued_async_packages[package_name], "Package %q is not being loaded", package_name)
	local resource_handle = Application.resource_package(package_name)
