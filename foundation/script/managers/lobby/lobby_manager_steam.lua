-- chunkname: @foundation/script/managers/lobby/lobby_manager_steam.lua

class("LobbyManagerSteam")
...
LobbyManagerSteam.init = function (self, options)
	...
	self._matchmaking_timeout = 0
	self.private_enabled = false
	self.unjoinable = true
	...
	self:set_network_hash(options.revision_check)

	self._members = {}
	self._refreshed_server_index = -1
	self._blacklisted_lobbies = {}
	self.game_tags = {}
end

LobbyManagerSteam.enable_matchmaking = function (self, enable)
	self._matchmaking = enable
	self._matchmaking_timeout = Managers.time:time("main") + 5

	self:set_lobby_data("matchmaking", enable and "true" or "false")
end

LobbyManagerSteam.is_matchmaking = function (self)
	return self._matchmaking
end

LobbyManagerSteam.get_lobby_info = function (self, lobby_num)
	return self.client:lobby_browser():lobby(lobby_num)
end

LobbyManagerSteam.using_steam = function (self)
	return true
end

LobbyManagerSteam.set_private = function (self, private_enable)
	print("Private:", private_enable)

	self.private_enabled = private_enable

	self:set_lobby_data("private", self.private_enabled and "true" or "false")
end

LobbyManagerSteam.set_unjoinable = function (self, unjoinable)
	self.unjoinable = unjoinable

	self:set_lobby_data("unjoinable", self.unjoinable and "true" or "false")
end

LobbyManagerSteam.set_network_hash = function (self, do_revision_check, extra_hash)
	local trunk_revision = self.content_revision
	local engine_revision = Application.build_identifier()

	if do_revision_check then
...
LobbyManagerSteam.create_lobby = function (self, cb_lobby_created, lobby_max_members, no_chat)
	if not self.client then
		self:create_network_client()
	end

	self.lobby = Network.create_steam_lobby(self.LOBBY_TYPE, lobby_max_members or self.LOBBY_MAX_MEMBERS)
	self.hosting = true
...
LobbyManagerSteam.join_lobby_by_lobby_id = function (self, lobby_id)
	if not self.client then
		self:create_network_client()
	end

	self.lobby = Network.join_steam_lobby(lobby_id)
...
LobbyManagerSteam.reset = function (self)
	...
	self._matchmaking = false
	self.private_enable = false
	self.unjoinable = true
	self.state = LobbyState.OFFLINE
...
LobbyManagerSteam._update_members = function (self)
	if self.lobby then
		local old_members = self._members
		local members = self.lobby:members()

		for _, member in pairs(members) do
			if not self._members[member] and Managers.state.event then
				Managers.state.event:trigger("added_member_to_lobby", member)
				print("Added", member, "to lobby")

				old_members[member] = true
			end
		end

		for old_member, _ in pairs(old_members) do
			local exists = false
			...
			if not exists and Managers.state.event then
				print("Removed", old_member, "from lobby")
				Managers.state.event:trigger("removed_member_from_lobby", old_member)

				old_members[old_member] = nil
			end
		end
	end
end
...
LobbyManagerSteam.update = function (self, dt)
	...
	self:_update_lobby_state(dt)

	if self._matchmaking and self.state == LobbyState.JOINED and self.server and (not self:is_refreshing_lobby_browser() or Managers.time:time("main") > self._matchmaking_timeout) then
		self:refresh_lobby_browser()
...
LobbyManagerSteam._update_lobby_state = function (self, dt)
...
LobbyManagerSteam.blacklist_lobby = function (self, lobby_id)
	local t = Managers.time:time("main")

	self._blacklisted_lobbies[lobby_id] = t

	table.dump(self._blacklisted_lobbies, "BLACKLISTED_LOBBIES")
	self:refresh_lobby_browser()
end

local BLACKLIST_TIME = 300

LobbyManagerSteam.is_lobby_blacklisted = function (self, lobby_id)
	local blacklisted_at = self._blacklisted_lobbies[lobby_id]

	if not blacklisted_at then
		return false
	end

	local blacklisted_until = blacklisted_at + BLACKLIST_TIME
	local t = Managers.time:time("main")

	if blacklisted_until < t then
		self._blacklisted_lobbies[lobby_id] = nil
