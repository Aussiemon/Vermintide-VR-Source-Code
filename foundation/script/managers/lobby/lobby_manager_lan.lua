-- chunkname: @foundation/script/managers/lobby/lobby_manager_lan.lua

class("LobbyManagerLan")
...
LobbyManagerLan.init = function (self, options)
	...
	if options.revision_check then
		self.combined_hash = Application.make_hash(self.network_hash, trunk_revision, engine_revision, options.project_hash)
	else
	...
	print("-------------------------------------")
	printf("Combined Hash: %s", tostring(self.combined_hash))
	print("-------------------------------------")
end

LobbyManagerLan.enable_matchmaking = function (self, enable)
...
LobbyManagerLan.is_matchmaking = function (self)
	return self._matchmaking
end

LobbyManagerLan.get_lobby_info = function (self, lobby_num)
	return self.client:lobby_browser():lobby(lobby_num)
end

LobbyManagerLan.using_steam = function (self)
	return false
end

LobbyManagerLan.set_private = function (self, private_enable)
	self.private_enabled = private_enable

	self:set_lobby_data("private", self.private_enabled and "true" or "false")
end

LobbyManagerLan.set_unjoinable = function (self, unjoinable)
...
LobbyManagerLan.create_lobby = function (self, cb_lobby_created, lobby_max_members)
	if not self.client then
		self.client = Network.init_lan_client(self.config_file_name)
	end

	self.lobby = Network.create_lan_lobby(self.port, lobby_max_members or self.LOBBY_MAX_MEMBERS)

	fassert(self.lobby, "[LobbyManagerLan] Network.create_lobby returned nil")
	Network.set_max_transmit_rate(0.011111111111111112)
	...
	local cm = Managers.chat

	if cm then
		cm:register_channel(1, callback(self, "lobby_members"))
	end
...
LobbyManagerLan.is_refreshing_lobby_browser = function (self)
	return self.client:lobby_browser():is_refreshing()
end
...
LobbyManagerLan.lobby_browser_content = function (self)
	...
	for i = 1, self.client:lobby_browser():num_lobbies() do
		local lobby_data = self.client:lobby_browser():lobby(i)

		lobby_data.lobby_num = i
...
LobbyManagerLan._update_members = function (self)
	if self.lobby then
		local old_members = self._members
		local members = LanLobby.members(self.lobby)
...
LobbyManagerLan._update_lobby_state = function (self, dt)
...
LobbyManagerLan.blacklist_lobby = function (self, lobby_id)
	return
end

LobbyManagerLan.is_lobby_blacklisted = function (self, lobby_id)
	return false
end
